        ...
        uint16_t send_wq_size, struct ibv_cq *s_cq)
{
    ...
    init_attr_ex.send_ops_flags |= IBV_QP_EX_WITH_VECTOR_CALC;
...
int main(int argc, char *argv[])
{
    ...
    if (wc.status != 0 || wc.opcode != IBV_WC_RDMA_WRITE) {
        printf("send qp scq get unexpected CQE\n");
    } else {
        printf("send qp scq get expected CQE\n");
    }

    printf("start checking recv qp rcq\n");
    while (ibv_poll_cq(recv_qp_rcq, 1, &wc) != 0) break;
    if (wc.status != 0 || wc.imm_data != 0xdeadbeef) {
        printf("recv qp rcq get unexpected CQE\n");
    } else {
        printf("recv qp rcq get expected CQE\n");
    }
