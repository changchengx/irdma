...
static int configure_sig_mkey(struct resources *res, int src_idx, struct mlx5dv_sig_block_attr *sig_attr)
{
    ...
    uint32_t access_flags = IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ;
...
uint32_t qp_self_connected(struct ibv_qp* qp)
{
    ...
    struct ibv_qp_attr qpa = {
        ...
        .max_dest_rd_atomic = MAX_CRC_TASK_SIZE,
        .min_rnr_timer = 0x12
    };
    ...
    qpa.rnr_retry  = 0;
    qpa.sq_psn     = 0;
    qpa.max_rd_atomic  = MAX_CRC_TASK_SIZE;
    ...
    while (state.KeepRunningBatch(BATCH)) {
        for (int i = 0; i < BATCH; ++i) {
            ...
            case 16: qpx->wr_id = 16; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[15]->lkey, 0, 4096);
            case 15: qpx->wr_id = 15; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[14]->lkey, 0, 4096);
            case 14: qpx->wr_id = 14; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[13]->lkey, 0, 4096);
            case 13: qpx->wr_id = 13; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[12]->lkey, 0, 4096);
            case 12: qpx->wr_id = 12; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[11]->lkey, 0, 4096);
            case 11: qpx->wr_id = 11; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[10]->lkey, 0, 4096);
            case 10: qpx->wr_id = 10; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[9]->lkey, 0, 4096);
            case  9: qpx->wr_id =  9; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[8]->lkey, 0, 4096);
            case  8: qpx->wr_id =  8; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[7]->lkey, 0, 4096);
            case  7: qpx->wr_id =  7; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[6]->lkey, 0, 4096);
            case  6: qpx->wr_id =  6; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[5]->lkey, 0, 4096);
            case  5: qpx->wr_id =  5; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[4]->lkey, 0, 4096);
            case  4: qpx->wr_id =  4; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[3]->lkey, 0, 4096);
            case  3: qpx->wr_id =  3; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[2]->lkey, 0, 4096);
            case  2: qpx->wr_id =  2; qpx->wr_flags = 0;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[1]->lkey, 0, 4096);
            case  1: qpx->wr_id =  1; qpx->wr_flags = IBV_SEND_SIGNALED;
                ibv_wr_rdma_read(qpx, res.src_data_mr->rkey, (uintptr_t)res.src_data_mr->addr);
                ibv_wr_set_sge(qpx, res.sig_mkey[0]->lkey, 0, 4096);
            }
