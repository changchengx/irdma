...
/* $ gcc -o dma_crc dma_crc32c.c -libverbs -lmlx5
 * $ ./dma_crc mlx5_0
 */
...
void check_support_dma_crc32c(struct ibv_context *ctxt)
{
    ...
    if (mlx5dv_query_device(ctxt, &attrs_out) || !(attrs_out.comp_mask & MLX5DV_CONTEXT_MASK_WR_MEMCPY_LENGTH)) {
        printf("failed to query mmo dma max length\n");
        exit(__LINE__);
    }

    printf("max_wr_memcpy_length: %lu\n", attrs_out.max_wr_memcpy_length);
}
...
int main(int argc, char *argv[])
{
    ...
    check_support_dma_crc32c(res.ib_ctx);
...
#if 1
    mlx5dv_wr_memcpy(mqpx, res.sig_mkey->lkey, 0, res.src_data_mr->lkey, (uint64_t)res.src_data_mr->addr, 4096);
#else
    //mlx5dv_wr_memcpy(mqpx, res.dst_data_mr->lkey, (uint64_t)res.dst_data_mr->addr, res.src_data_mr->lkey, (uint64_t)res.src_data_mr->addr, 4096);
#endif
    ...
    if (poll_completion(res.qp->send_cq, IBV_WC_DRIVER3))
        return -1;
    ...
    sge.addr = (uintptr_t)res.src_data_mr->addr;
    sge.length = 4096;
