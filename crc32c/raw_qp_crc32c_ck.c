...
/* Build:
 * $ gcc -o raw_qp_crc32c_ck raw_qp_crc32c_ck.c -libverbs -lmlx5
 *
 * Server env:
 ...
 * Run:
 *  Server
 *   $ sudo ./raw_qp_crc32c_ck --port=20258 --ib-dev=mlx5_0 --local_mac a0:88:c2:f5:dd:1a --remote_mac a0:88:c2:f5:ee:9e --local_ip 192.168.33.15 --remote_ip 192.168.33.17 --local_port 8976 --remote_port 6789 --server
 *
 *  Client
 *   $ sudo ./raw_qp_crc32c_ck --port=20258 --ib-dev=mlx5_6 --local_mac a0:88:c2:f5:ee:9e --remote_mac a0:88:c2:f5:dd:1a --local_ip 192.168.33.17 --remote_ip 192.168.33.15 --local_port 6789 --remote_port 8976 --client
 */
...
static int configure_sig_mkey(struct pingpong_context *ctx, enum sig_mode mode, struct mlx5dv_sig_block_attr *sig_attr)
{
    ...
    if (mode == SIG_MODE_INSERT_ON_WIRE) {
        struct ibv_sge sge;
        sge.addr = (uintptr_t)ctx->mr->addr + 42;
        ...
        /* data */
        mr_interleaved[0].addr = (uintptr_t)ctx->mr->addr + 42;
...
static int reg_sig_mkey(struct pingpong_context *ctx, enum sig_mode mode)
{
    ...
    if (configure_sig_mkey(ctx, mode, &sig_attr))
        return -1;
...
int main(int argc, char *argv[])
{
    ...
    if (param.machine == 0) {
        ...
        sge[1].length = 512 + 4;
        sge[1].lkey = ctx.sig_mkey->lkey;
        memset(ctx.buf, 0, ctx.param->buff_size);
