...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < size; i++) {
        hostSendBuff[i] = myRank * size + i;
    }
    CUDACHECK(cudaMemcpy(sendbuff, hostSendBuff, size * sizeof(float), cudaMemcpyHostToDevice));

    CUDACHECK(cudaMalloc((void**)&recvbuff, nRanks * size * sizeof(float)));
    CUDACHECK(cudaMemset(recvbuff, 0, nRanks * size * sizeof(float)));
    ...
    float* hostRecvBuff = (float*)malloc(nRanks * size * sizeof(float));
    CUDACHECK(cudaMemcpy(hostRecvBuff, recvbuff, nRanks * size * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for(int rank_idx = 0; rank_idx < nRanks; rank_idx++){
        for (int i = 0; i < size; i++) {
            printf("%f ", hostRecvBuff[rank_idx * size + i]);
        }
        printf("\n");
    }
    printf("\n");
