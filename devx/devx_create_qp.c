...
static void devx_query_qp(struct ibv_qp *qp)
{
#ifdef DEVX
    uint32_t out_qp[DEVX_ST_SZ_DW(query_qp_out)] = {};
    mlx5dv_wrap_devx_query_qp(qp, out_qp, sizeof(out_qp));
    printf("qp:0x%06x cd_master:%d\n", qp->qp_num, DEVX_GET(query_qp_out, out_qp, qpc.cd_master));
#endif
}
...
#else
    rc = mlx5dv_wrap_devx_modify_qp_init2rtr(qp, &qp_attr, qp_attr_mask);
    devx_query_qp(qp);
#endif
    if (rc) {
...
static int modify_qp_rtr2rts(struct ibv_qp *qp)
{
    ...
    rc = ibv_modify_qp(qp, &qp_attr, qp_attr_mask);
#else
    rc = mlx5dv_wrap_devx_modify_qp_rtr2rts(qp, &qp_attr, qp_attr_mask);
    devx_query_qp(qp);
#endif
    if (rc) {
...
#else
    printf("no specific RDMA_WRITE_WITH_IMM implementation for the created DEVX QP\n");
#endif
}
...
#else
    mlx5_devx_post_rdma_write(qp, remote_con_data.rkey,
            remote_con_data.addr + server * sizeof(uint32_t),
            mr->lkey, (uint64_t)data_chunk + server * sizeof(uint32_t), sizeof(uint32_t));
    mlx5dv_devx_ring_db(qp);
#endif
    sync_fence();
...
#else
    struct ibv_qp *qp = mlx5dv_wrap_devx_create_qp(ctxt, &init_attr_ex, &dv_attr);
    devx_query_qp(qp);
#endif
