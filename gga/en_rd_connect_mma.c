 ...
 *
 * # server RDMA_READ data from client
 * # gcc -I${RDMA_PATH}build/include -L${RDMA_PATH}build/lib en_rd_connect_mma.c -libverbs -lmlx5 -o en_rd_connect_mma
 * # server(192.168.30.21) taskset -c 8 ./en_rd_connect_mma mlx5_2 # use "-i" to inv cache w/o writeback, use "-f" to not ctrl rdma_read speed
 * # client(192.168.30.20) taskset -c 8 ./en_rd_connect_mma mlx5_2 -n 192.168.30.21
 * # run perf on server: taskset -c 15 ./bfperf_bf3_pmc_v1.py -v -s 1
 */
...
uint32_t normal_traffic(struct ibv_qp *qp, struct ibv_mr *local_mr, struct ibv_mr *remote_mr,
		uint32_t offset, uint32_t length, uint32_t is_server)
{
...
uint32_t local_cache_invalid(struct ibv_qp *qp, struct ibv_mr *mr, uint32_t offset, uint32_t length, bool need_writeback)
{
	...
	mlx5dv_wr_invcache(mqpx, mr->lkey, (uint64_t)mr->addr + offset, length, need_writeback);
...
{
	printf("\n"
	       "-p <port> : listen or connect port\n"
	       "\n"
	       "client: # ./en_rd_connect_mma <dev_name> -n <sever_ip_addr>\n"
	       "\n"
	       "server: '-f :' do not control RDMA_READ speed. Defaut: control RDMA_READ speed\n"
	       "        '-i :' invalidate L3 cached data after using RDMA_READ to read data from client to server, default: do not invalidate\n"
	       "        '-s <msg_size> :' server read data with the length <msg_size> from client to server, default: 2MB\n"
	       "        '-I <iteration_times> :' In one iteration, server read 16MB from client to server, default: 40000\n"
	       " example:\n"
	       "        # ./en_rd_connect_mma <dev_name> -f\n"
	       "        # ./en_rd_connect_mma <dev_name> -i\n"
	       "        # ./en_rd_connect_mma <dev_name> -s 1048576 -I 30000   \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 65536 -I 4000      \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 65536 -I 30000 -f  \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 8192 -I 500        \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 8192 -I 10000 -f   \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 4096 -I 250        \n"
	       "        # ./en_rd_connect_mma <dev_name> -s 4096 -I 6000 -f    \n"
	       );
}
...
int parse_cmd(int argc, char * const argv[], cmd_args_t *args)
{
	...
	args->msg_size     = MR_LEN;
	args->n_iterations = 40000;

	while ((c = getopt(argc, argv, "hiw6:n:ps:I:f")) != -1) {
		switch (c) {
		case 'h':
			usage();
			exit(0);
		...
		case 's':
			...
			if (args->msg_size > MR_LEN) {
				fprintf(stderr, "msg size can't be larger than 0x%08x\n", MR_LEN);
				exit(__LINE__);
			}
			printf("traffic and invalidate msg_size = 0x%08x\n", args->msg_size);
			break;
		case 'I':
			args->n_iterations = atoi(optarg);
			printf("traffic repeat %d times\n", args->n_iterations);
			break;
		}
	}
...
void server_inv_cache_or_sleep(struct ibv_qp *qp, struct ibv_mr *mr, uint32_t offset, uint32_t length,
		int is_server, int no_sleep, int inv_cache, uint32_t need_writeback)
{
	if (!is_server)
		return;
	...
	if (!inv_cache) {
		...
		local_cache_invalid(qp, mr, offset, length, need_writeback);
...
int main(int argc, char *argv[])
{
	...
	struct timeval start, end, elpase;
	gettimeofday(&start, NULL);
	for (uint32_t i = 0; i < cmd_args.n_iterations; i++) {
		for (uint32_t j = 0; j < 8; j++) {
			uint32_t msg_size = cmd_args.msg_size;
			for (uint32_t k = 0; k < MR_LEN / msg_size; k++) {
				uint32_t offset_addr = j * MR_LEN + msg_size * k;
				normal_traffic(qp, mr, &remote_mr, offset_addr, msg_size, is_server);

				server_inv_cache_or_sleep(inv_cache_qp, mr, offset_addr, msg_size,
						is_server, cmd_args.no_sleep, cmd_args.server_inv_cache, cmd_args.mma_need_writeback);
			}
		}
	}

	server_send_ack(oob_sock, is_server, 15);
	client_wait_ack(oob_sock, is_server, 15);
