 ...
 *
 * $ gcc -I${RDMA_PATH}build/include -L${RDMA_PATH}build/lib mma_func_ver.c -libverbs -lmlx5 -o mma_func_ver
 * $ ./mma_func_ver mlx5_2
 */
...
uint32_t local_cache_invalid(struct ibv_qp *qp, struct ibv_mr *mr, uint64_t offset)
{
	...
	uint64_t addr = (uint64_t)mr->addr;

	ibv_wr_start(qpx);
	qpx->wr_id = 0x33219280;
	qpx->wr_flags = IBV_SEND_SIGNALED;

	*write_addr = 0xdeadbeefcafeabcd; // write data into cache, this is old data
	sw_flush_cache(); // evict the data into DDR
	*write_addr = 0xabcdcafebeefdead; // write data into cache, this is new data
	mlx5dv_wr_invcache(mqpx, mr->lkey, addr, MAX_INV_CACHE_LENGTH, 0); // no writeback
	...
	if (*write_addr == 0xdeadbeefcafeabcd) {
		// invalidate the cache without writeback, got old data
		printf("passed GGA/MMA at offset = 0x%016lx invalidate without writeback test\n", offset);
	} else {
		if (retry != 2) {
			retry++;
			local_cache_invalid(qp, mr, offset);
		} else {
			printf("GGA MMA failed at offset = 0x%016lx under inv cache w/o writeback\n", offset);
			exit(__LINE__);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	struct ibv_pd* pd = ibv_alloc_pd(ibv_ctx);
	struct ibv_mr* mr = create_one_mr(pd, MAX_INV_CACHE_LENGTH);
	...
	for (uint64_t offset = 0; offset < MAX_INV_CACHE_LENGTH; offset = offset + 8) {
		local_cache_invalid(qp, mr, offset); // no writeback
	}
	return 0;
}
