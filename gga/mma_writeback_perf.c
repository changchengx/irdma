 ...
 *
 * $ gcc -I${RDMA_PATH}build/include -L${RDMA_PATH}build/lib mma_writeback.c -libverbs -lmlx5 -o mma_writeback
 * $ # invalidate cache w/ writeback./mma_writeback mlx5_2 -w
 * $ # invalidate cache w/o writeback./mma_writeback mlx5_2
 */
...
uint32_t local_cache_invalid(struct ibv_qp *qp, struct ibv_mr *mr, uint32_t length, uint32_t need_writeback)
{
	...
	mlx5dv_wr_invcache(mqpx, mr->lkey, (uint64_t)mr->addr, MR_LEN, need_writeback); // inv cache w/o writeback
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < 800000; i++) {
		...
		local_cache_invalid(qp, mr, MR_LEN, need_writeback); // GGA/MMA rq_mr[0, MR_LEN}
	}
