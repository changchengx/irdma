 ...
 * # gcc -I${RDMA_PATH}build/include -L${RDMA_PATH}build/lib rd_connect_mma.c -libverbs -lmlx5 -o rd_connect_mma
 * # server(192.168.30.21) taskset -c 8 ./rd_connect_mma mlx5_2 # use "-i" to inv cache w/o writeback, use "-i -w" to inv cache w/ writeback
...
int parse_cmd(int argc, char * const argv[], cmd_args_t *args)
{
	...
	while ((c = getopt(argc, argv, "iw6:n:p")) != -1) {
		switch (c) {
		case 'n':
...
void server_inv_cache_or_sleep(struct ibv_qp *qp, struct ibv_mr *mr, uint32_t offset, int is_server, int inv_cache, uint32_t need_writeback)
{
	if (!is_server)
		return;
	...
	if (!inv_cache) {
		...
		local_cache_invalid(qp, mr, offset, need_writeback);
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < 40000; i++) {
		for (uint32_t j = 0; j < 8; j++) {
			// server RDMA_READ client's mr[MR_LEN * j, MR_LEN * j + MR_LEN} to
			//                  server's mr[MR_LEN * j, MR_LEN * j + MR_LEN}
			normal_traffic(qp, mr, &remote_mr, MR_LEN * j, is_server);

			// server sleep or invalidate cache for mr[MR_LEN * j, MR_LEN * j + MR_LEN}
			server_inv_cache_or_sleep(inv_cache_qp, mr, MR_LEN * j , is_server, cmd_args.server_inv_cache, cmd_args.mma_need_writeback);
		}
	}
