 ...
 *
 * # client RDMA_WRITE data to server
 * # gcc -I${RDMA_PATH}build/include -L${RDMA_PATH}build/lib wr_connect_mma.c -libverbs -lmlx5 -o wr_connect_mma
 * # server(192.168.30.21) taskset -c 8 ./wr_connect_mma mlx5_2 # use "-i" to invalide cache
 * # client(192.168.30.20) taskset -c 8 ./wr_connect_mma mlx5_2 -n 192.168.30.21
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < 40000; i++) {
		for (uint32_t j = 0; j < 8; j++) {
			// client mr[MR_LEN * j, MR_LEN * j + MR_LEN} RDMA_WRITE to
			// server mr[MR_LEN * j, MR_LEN * j + MR_LEN};
			normal_traffic(qp, mr, &remote_mr, MR_LEN * j, is_server);
			fence(oob_sock, is_server, i + 8 * j);

			// server sleep or invalidate cache for mr[MR_LEN * j, MR_LEN * j + MR_LEN}
			server_inv_cache_or_sleep(inv_cache_qp, mr, MR_LEN * j , is_server, cmd_args.server_inv_cache);
			fence(oob_sock, is_server, i + 8 * j + 1024);
		}
	}
