 ...
 * Output:
 *  out_ev
 *    The event will be passed back to the caller, if desired
 ...
 *
 * Description:
 *  Waits for the next CM event and check this matches the expected type.
 */
...
int create_resources(struct context *ctx)
{
	...
	/* If we are bound to an address, then a PD was already allocated to the CM ID */
	if (!ctx->pd) {
...
int main(int argc, char** argv)
{
	...
	printf("joined dgid: %s, mlid 0x%x, sl %d\n", buf, event->param.ud.ah_attr.dlid, event->param.ud.ah_attr.sl);
