...
#define QP_COUNT (1024)

struct ibv_cq *cq[QP_COUNT];
struct ibv_qp *qp[QP_COUNT];

/* structure to exchange data which is needed to connect to QPs */
struct cm_con_data_t {
    uint64_t addr;   /* Buffer address */
    uint32_t rkey;   /* Remote key */
    uint32_t qp_num[QP_COUNT]; /* QP number */
...
static void init_local_con_data(struct ibv_context* ctxt)
{
    struct ibv_port_attr port_attr = {};
    if (ibv_query_port(ctxt, 1, &port_attr)) {
        printf("failed to query port1 attr\n");
    }

    union ibv_gid local_gid = {};
    if (ibv_query_gid(ctxt, 1, 3, &local_gid)) {
        printf("failed to query port1 gid3\n");
    }
	...
	for (int i = 0; i < QP_COUNT; i++) {
		local_con_data.qp_num[i] = htonl(qp[i]->qp_num);
	}
}
...
static int modify_qp_init2rtr(struct ibv_qp *qp[], int idx)
{
    struct ibv_qp_attr qp_attr = {};
    int qp_attr_mask;
    int rc;
    ...
    qp_attr.dest_qp_num = remote_con_data.qp_num[idx];
    qp_attr.rq_psn = 0;
    ...
    rc = ibv_modify_qp(qp[idx], &qp_attr, qp_attr_mask);
    if (rc) {
        printf("failed to modify qp:0x%06x init2rtr\n", qp[idx]->qp_num);
    }
...
static int modify_qp_rts2err(struct ibv_qp *qp)
{
	struct ibv_qp_attr qp_attr = {};
	int rc = 0;
	...
	if (rc) {
		printf("failed to modify qp:0x%06x rts2err\n", qp->qp_num);
	}
...
void post_rdma_write_write_imm(struct ibv_qp* qp)
{
    ...
    if (server) {
        ...
        if (wc.status == IBV_WC_SUCCESS && data_chunk[!server] == 'C' && wc.imm_data == 0xcafebeef & wc.wr_id == 0xdeadbeef) {
            ...
            printf("server get unexpected data ==> %c & 0x%08x & 0x%016lx\n", data_chunk[!server], wc.imm_data, wc.wr_id);
        }
    } else {
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < QP_COUNT; i++) {
		cq[i] = ibv_create_cq(ctxt, 16, NULL, NULL, 0);
	}
	...
	for (int i = 0; i < QP_COUNT; i++) {
		...
		init_attr_ex.send_cq = cq[i];
		init_attr_ex.recv_cq = cq[i];
		...
		qp[i] = mlx5dv_create_qp(ctxt, &init_attr_ex, &dv_attr);
		modify_dv_qp_rst2init(qp[i]);

		if (qp[i] == NULL) {
			printf("failed to create qp[%d]\n", i);
		}
	}
    ...
    init_local_con_data(ctxt);

    get_remote_con_data();

    printf("local data:%p, local rkey:0x%08x, remote data:%p, remote rkey:0x%08x\n",
            data_chunk, mr->rkey, (void*)(remote_con_data.addr), remote_con_data.rkey);

	for (int i = 0; i < QP_COUNT; i++) {
		printf("local_qpn:0x%06x, remote_qpn:0x%06x\n", qp[i]->qp_num, remote_con_data.qp_num[i]);
	}

	for (int i = 0; i < QP_COUNT; i++) {
		modify_qp_init2rtr(qp, i);
		modify_qp_rtr2rts(qp[i]);
	}

	for (int i = 0; i < QP_COUNT; i++) {
		modify_qp_rts2err(qp[i]);
	}

	struct timespec tstart={0,0}, tend={0,0};
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	for (int i = 0; i < QP_COUNT; i++) {
		if (ibv_destroy_qp(qp[i]) != 0) {
			printf("failed to destroy qp:0x%06x\n", qp[i]->qp_num);
		} else {
			qp[i] = NULL;
		}
	}
	clock_gettime(CLOCK_MONOTONIC, &tend);
	printf("RTS2ERR %d QPs takes about %.6f seconds\n", QP_COUNT,
		((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec));

	for (int i = 0; i < QP_COUNT; i++) {
		if (ibv_destroy_cq(cq[i]) != 0) {
			printf("failed to destroy cq[%d]\n", i);
		}
	}
